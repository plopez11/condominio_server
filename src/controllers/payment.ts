...
const getPaymentsBybill = async (req: Request, res: Response) => {
            
    try {
        const bill_id = req.params.bill_id;
        const apartment_id = req.params.bill_id;
        let pagina = Number(req.query.pagina) || 1;
        if ( pagina === 0) pagina = 1;
        let skip = pagina - 1;
        skip = skip * 10;
            
        const payments: QueryResult = await pool.query('SELECT * FROM public.tmccs_payment where bill_id=$1 AND apartment_id=$2',[bill_id,apartment_id] );
        ...
        handleHttp(res,'ERROR_GET_PAYMENTS, ' + e);
    }
};
...
const ModifApartment = async (req: Request, res: Response) => {
    try {
        ...
        const apartment: QueryResult = await pool.query('UPDATE public.tmccs_apartments SET building_id=$1,apartmentNumber=$2,floor=$3,status=$4 WHERE id=$5',
                [body.building_id,body.apartmentNumber,body.floor,body.status,Id]);
...
const createApartment = async (req: Request, res: Response) => {    
    ...
    try {
        ...
        const apartment: QueryResult = await pool.query('INSERT INTO public.tmccs_apartments (building_id,apartmentNumber,floor,status) VALUES($1, $2, $3, $4)',
...
export {getPaymentsBybill, createApartment,getApartments, getApartmentsBuilding,ModifApartment, deleteApartment};
